...
class FramePart
  ...
  override def getStrength(mop: MovingObjectPosition, player: EntityPlayer) =
    player.getBreakSpeed(
      ...
      API.getFrameBlock.getBlockHardness(
        player.worldObj,
        mop.blockX,
        mop.blockY,
        mop.blockZ
  ...
  override def pickItem(hit: MovingObjectPosition) = new ItemStack(
    API.getFrameBlock
  )
...
object FrameBlockConverter extends IPartConverter {
  ...
  override def convert(world: World, pos: BlockCoord) =
    if (world.getBlock(pos.x, pos.y, pos.z) == API.getFrameBlock) new FramePart
    else null
}
...
object FMPTileHandler extends ITileMover {
  ...
  override def postMove(w: World, x: Int, y: Int, z: Int) =
    WorldLib.uncheckedGetTileEntity(w, x, y, z) match {
